<?php
/* $Id: Stocks.php 7112 2015-01-31 18:10:59Z rchacon $ */
...
if (isset($_POST['submit'])) {
	...
	if (($_POST['MBFlag']=='A'
		...
		$InputError = 1;
		prnMsg(_('Assembly/package material/Phantom/Service/Labour items cannot also be controlled items') . '. ' . _('Assemblies/Dummies/Phantom and package material are not physical items and batch/serial control is therefore not appropriate'),'error');
		$Errors[$i] = 'Controlled';
		$i++;
	}
...
$sql1 = "SELECT categoryid, categorydescription FROM ProductionBlock";
$sql2 = "SELECT categoryid, categorydescription FROM MarketingDivision";
$ErrMsg = _('The stock categories could not be retrieved because');
$DbgMsg = _('The SQL used to retrieve stock categories and failed was');
$result = DB_query($sql,$ErrMsg,$DbgMsg);
$result1 = DB_query($sql1,$ErrMsg,$DbgMsg);
$result2 = DB_query($sql2,$ErrMsg,$DbgMsg);
...
echo '<tr>
		<td>' . _('Production Block') . ':</td>
		<td><select name="ProductionID " maxlength="12"><option selected="selected" value=""></option>';
		
	while ($myrow=DB_fetch_array($result1)){
	if (!isset($_POST['ProductionID']) OR  $myrow['categoryid']==$_POST['ProductionID']){
		echo '<option value="'. $myrow['categorydescription'] . '">' . $myrow['categorydescription'] . '</option>';
	} else {
		echo '<option value="'. $myrow['categorydescription'] . '">' . $myrow['categorydescription'] . '</option>';
	}
	$ProductionBlock=$myrow['categorydescription'];
}

if (!isset($_POST['ProductionID'])) {
	$_POST['categorydescription']=$ProductionBlock;
}

echo '</select><a target="_blank" href="'. $RootPath . '/ProductionBlock.php">' . _('Add or Modify Stock Production Block') . '</a></td>
	</tr>';



	echo '<tr>
		<td>' . _('Marketing Division') . ':</td>
		<td><select name="MarketingID" "><option selected="selected" value="" ></option>';
		
	while ($myrow=DB_fetch_array($result2)){
	if (!isset($_POST['MarketingID']) OR  $myrow['categoryid']==$_POST['MarketingID']){
		echo '<option  value="'. $myrow['categorydescription'] . '">' . $myrow['categorydescription'] . '</option>';
	} else {
		echo '<option value="'. $myrow['categorydescription'] . '">' . $myrow['categorydescription'] . '</option>';
	}
	$MarketingDivision=$myrow['categorydescription'];
}

if (!isset($_POST['MarketingID'])) {
	$_POST['categorydescription']=$MarketingDivision;
}

echo '</select><a target="_blank" href="'. $RootPath . '/MarketingDivision.php">' . _('Add or Modify Marketing Division') . '</a></td>
	</tr>';
...
if (!isset($_POST['MBFlag']) OR  $_POST['MBFlag']=='P'){
	echo '<option selected="selected" value="P">' . _('Package Material') . '</option>';
} else {
	echo '<option value="P">' . _('Package Material') . '</option>';
}
if (!isset($_POST['MBFlag']) OR  $_POST['MBFlag']=='M'){
	echo '<option selected="selected" value="M">' . _('Finished Product') . '</option>';
} else {
	echo '<option value="M">' . _('Finished Product') . '</option>';
}
...
if (!isset($_POST['MBFlag']) OR  $_POST['MBFlag']=='B' OR !isset($_POST['MBFlag']) OR $_POST['MBFlag']==''){
	echo '<option selected="selected" value="B">' . _('Raw Material') . '</option>';
} else {
	echo '<option value="B">' . _('Raw material') . '</option>';
}

if (isset($_POST['MBFlag']) AND $_POST['MBFlag']=='D'){
	echo '<option selected="selected" value="D">' . _('Service/Labour') . '</option>';
} else {
	echo '<option value="D">' . _('overheadcost') . '</option>';
}
